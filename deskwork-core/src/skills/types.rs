...
impl SkillFile {
    pub fn from_markdown(path: impl AsRef<Path>, markdown: &str) -> Self {
        ...
        Self {
            name: frontmatter.get("name").cloned().unwrap_or_else(|| "unknown-skill".to_string()),
...
impl CommandFile {
    pub fn from_markdown(path: impl AsRef<Path>, plugin_id: impl Into<String>, markdown: &str) -> Self {
        let path_ref = path.as_ref();
        let (frontmatter, body) = parse_frontmatter(markdown);
        let stem = path_ref.file_stem().and_then(|s| s.to_str()).unwrap_or("unknown-command").to_string();
